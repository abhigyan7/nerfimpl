...
def render_image(nerf, pixel_locs):
    locations = jax.vmap(jax.vmap(lambda x: positional_encoding(x,10, scale=128.0)))(pixel_locs)
    rgbs = eqx.filter_vmap(
        eqx.filter_vmap(nerf)) (locations)
    return rgbs
...
def optimize_one_batch(nerf, pixel_locs, rgb_ground_truths, optimizer, optimizer_state):
    ...
    def loss_fn(nerf, pixel_locs, rgb_ground_truths):
        locations = jax.vmap(lambda x: positional_encoding(x,10,scale=128.0))(pixel_locs)
